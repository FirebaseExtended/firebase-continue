...
+ (void)initialize {
  activityTypeForHandoff = [NSString stringWithFormat:@"%@.%@",
...
+ (void)broadcastToContinueActivityWithUrl:(NSString*)activityUrl
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
    ...
    [mostRecentActivityRef removeValueWithCompletionBlock:
     ^(NSError *_Nullable error, FIRDatabaseReference *_Nonnull ref) {
       ...
       [ref setValue:activity withCompletionBlock:
        ^(NSError * _Nullable error, FIRDatabaseReference * _Nonnull ref) {
          if (error != nil) {
            [self executeCompletionBlockOnMainThread:completionBlock
                                           withError:[self createErrorWithReason:
          ...
          // improved. For example, possibly look into the user experience of Apple Handoff - figure
          // out how to make it work well in the context of Firebase Continue (ex. invalidate an
          // Apple Handoff Activity if the user opens it in the browser, etc.)
          // This could also mean simply making Firebase Continue more Apple Handoff-like overall
          // (i.e. removing the concept of activity "staleness" and forcing the mobile clients to
          // manage whether or not the user may wish to continue something). This requires some
          // thought, so discussion on GitHub would be wise.
          lastBroadcastActivityForHandoff = [[NSUserActivity alloc]
                                             initWithActivityType:activityTypeForHandoff];
